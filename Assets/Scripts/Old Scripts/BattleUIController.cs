...
public class BattleUIController : MonoBehaviour
{
    ...
    [SerializeField]
    private UnityEngine.UI.Button[] actionButtons;
    [SerializeField]
    ...
    //Turn spell panel on/off
    public void ToggleSpellPanel(bool state) {
        spellPanel.SetActive(state);
        if(state == true) {
            BuildSpellList(BattleController.Instance.characters[0][BattleController.Instance.characterTurnIndex].spells);
        }
    }
    ...
    public void Defend() {
        BattleController.Instance.GetCurrentCharacter().Defend(1);
    }
}
